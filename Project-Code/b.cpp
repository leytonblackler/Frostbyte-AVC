...
int wall_dist= 450; //distance from wall by ir value that we want the bot to stay
int ir_error=0;
...
if(wall<35){
	if (front > 50 && left < 50) { //--Line in front and no line to the left.
		...
		if (proportional_signal < 0) { //--Too far right, need to turn left.
			...
			right_motor_speed = (70 - (proportional_signal / (119)) * 175);
			left_motor_speed = (70 + (proportional_signal / (119)) * 175); //((175 / maximum_error_code*kP)*proportional_signal));
		}
	}
...
if(ir_left>1000){wall=wall+1;}
}else if (wall=35){
//front high and left high
if(ir_left>300 && ir_front>ir_front_val){
    right_motor_speed=-90;
    left_motor_speed=90;
}
//front high left low
else if(ir_front>ir_front_val && ir_left<300){
    right_motor_speed= 90;
    left_motor_speed= -90;
}
//left high front low
else if(ir_left>450 && front<ir_front_val){
//Left motor is set to a proportional faster speed than the right motor.
			right_motor_speed = (70 - (ir_proportional / (119)) * 175); //(80+((175 / maximum_error_code*kP)*(proportional_signal*-1)));
			left_motor_speed = (70 + (ir_proportional / (119)) * 175);
}
//left mid front low
else if(ir_left<450 &&ir_left>200 && ir_front<ir_front_val){
//Right motor is set to a proportional faster speed than the left motor.
			right_motor_speed = (70 - (ir_proportional / (119)) * 175);
			left_motor_speed = (70 + (ir_proportional / (119)) * 175); //((175 / maximum_error_code*kP)*proportional_signal));
		
}else if(ir_left<200 && ir_front<ir_front_val){
 right_motor_speed=90;
left_motor_speed=30;
}
