...
int processed_camera_output[239];
int pcor[240]; 		//Used to store the processed camera output (1 for line detected, 0 for white).
...
//bool for if there is a line to left and  no line in front
bool leftBool;
bool frontBool;
int left=0;
int front=0;
...
//	printf("Opening gate...");
	...
	while (run) {
		...
		process_picture();
                    ...
                    if (left>100){
                      leftBool=true;
                        ...
                        for(int i= 0;i<240;i++){
                         front= front + processed_camera_output[i];
                        }
                if(front>100){
                frontBool=true;
                }
		...
		for (int i = 0; i < 239; i++) {
			error_code_array[i] = pcor[i] * offset_position[i];
		}
...
/*        else if(!frontBool && leftBool){
		 right_motor_speed = (80); //(80+((175 / maximum_error_code*kP)*(proportional_signal*-1)));
                left_motor_speed = (0);
		}
*/
